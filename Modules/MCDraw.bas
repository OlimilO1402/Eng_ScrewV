...
'    CInch = aMM / 25.4
'End Function
Public Sub DrawSystem(c As cCairoContext, aSVFL As SchrVerbFL, ByVal X_m As Double, ByVal Y_m As Double, Optional ByVal bIsSchraubenloch As Boolean = False) 's As Schraube, sl As Schraubenloch, sg As Schraubengruppe, bl As Blech, br As Blech)
    ...
    With c
        ...
        .SetLineWidth 0.2, RoundToFullPixels:=True
        .SetLineCap CAIRO_LINE_CAP_SQUARE
    End With
    With aSVFL
        Dim sl As Schraubenloch: Set sl = IIf(bIsSchraubenloch, .Schraubenloch, Nothing)
        If Not .Schraube Is Nothing Then
            If .Schraubengruppe Is Nothing Then
                DrawSchrGarnitur c, .Schraube, .Schraubenloch, X_m, Y_m
            Else
                DrawSchraubengruppe c, .Schraubengruppe, X_m, Y_m
                If Not .BlechLinks Is Nothing Then
                    DrawBlechL c, .Schraubengruppe, .BlechLinks, X_m, Y_m
                End If
                If Not .BlechRechts Is Nothing Then
                    DrawBlechR c, .Schraubengruppe, .BlechRechts, X_m, Y_m, 2 * X_m
                End If
            End If
        End If
    End With
